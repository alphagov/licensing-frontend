...
def test_list_all_licences_returns_200_ok(client, mock_get_all_licences_from_db):
    mock_get_all_licences_from_db.return_value = []
    response = client.get(reverse("get_all_licences"))
    assert response.status_code == 200


def test_get_all_licences_from_database(client, mock_get_all_licences_from_db):
